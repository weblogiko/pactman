...
class Interaction:
    ...
    def set_versioned_provider_state(self, setup_url, var, state):
        ...
        kwargs = dict(
            json={"provider": self.pact.provider, "consumer": self.pact.consumer, var: state}
        )
        ...
        try:
            ...
            try:
                ...
                reason = str(e)
            return self.result.fail(
                f"Unable to configure provider state {state!r} at {setup_url}: {reason}"
            )
...
class ResponseVerifier:
    ...
    def verify(self, response):
        ...
        if self.status is not MISSING and response.status_code != self.status:
            ...
            return self.result.fail(
                f"{self.interaction_name} status code {response.status_code} is not "
    ...
    def find_rule(self, path):
        ...
        display = [
            (weighted_rule.weight, weighted_rule.rule.path) for weighted_rule in weighted_rules
        ]
...
class RequestVerifier(ResponseVerifier):
    ...
    def verify_query(self, spec_query, request):
        ...
        if self.pact.semver.major > 1 and self.matching_rules.get("query"):
            ...
            return self.result.fail(
                f"Request query {request_query!r} does not match expected {spec_query!r}"
            )
        return True
